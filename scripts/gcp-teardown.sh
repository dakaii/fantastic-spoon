...
#   DELETE_PROJECT  1 = also delete the GCP project (local mode only)
#   SKIP_ADC_CHECK  1 = skip ADC email vs gcloud account check
set -euo pipefail
...
# --- Ensure tfvars so destroy modules are not skipped ------------------------
export SKIP_AUTH=1
...
case "$MODE" in
  ...
  local)
    ...
    log "Verify: gcloud compute instances list --project=${GCP_PROJECT}"
    ;;
esac
